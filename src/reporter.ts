...
export async function postPRComment(
  ...
  const hasBlockingFinding = reports.some(
    (r) => (r.severity === 'CRITICAL' || r.severity === 'HIGH') && r.confidence_score >= 80
  );

  if (hasBlockingFinding) {
    // Auto-Remediation / Blocker pattern: use a PR Review to actively block the merge
